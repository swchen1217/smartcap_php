...
if($user!=""){
    if($starttime!=""){
		...
		while(list($starttime2,$usetime2,$ngtimes2)=mysqli_fetch_row($rs))
		{
			...
			$m = $t%60;
			$h = ($t-$m)/60;
			echo "<tr bgcolor='#FFFFFF'><td>".$starttime2."</td><td>".$h.":".$m.":".$s."</td><td>".$ngtimes2."</td><td>".($ngtimes2==0?"-":intval($usetime2/$ngtimes2))."</td><td>".($ngtimes2==0?'-':("<a href=".'"'."./ngslist.php?user=".$user."&starttime=".$starttime2.'"'.">表格</a> <a href=".'"'."./ngslinechart.php?user=".$user."&starttime=".$starttime2.'"'.">折線圖</a>"))."</td></tr>";
		}
        echo "</table>";
	//}
}
