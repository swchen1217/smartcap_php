...
if($user != "")
{
    ...
    $sql2 = "select starttime,usetime,ngtimes from mainlogtb where user = '".$user."'";
    ...
    if(mysqli_num_rows($rs2)>1){
        ...
        for($k=0;$k<mysqli_num_rows($rs);$k++){
            $sql3 = "select starttime,usetime,ngtimes from mainlogtb where user = '".$user_list[$k]."'";
            $rs3 = mysqli_query($con,$sql3);
            if(mysqli_num_rows($rs3) == 0)
                exit;
            $m=0;
            while(list($starttime,$usetime,$ngtimes)=mysqli_fetch_row($rs3)){
                if($m+1>$i)
                    break;
                $tmp=0;
                if($ngtimes!=0){
                    $ngs_sum=0;
                    $sql5 = "select ngsecond from slogtb where user = '".$user_list[$k]."' and starttime = '".$starttime."'";
                    $rs5 = mysqli_query($con,$sql5);
                    while(list($ngsecond)=mysqli_fetch_row($rs5))
                    {
        ...
        if($ngtimes!=0){
            $ngs_sum=0;
            $sql6 = "select ngsecond from slogtb where user = '".$user."' and starttime = '".$starttime."'";
            $rs6 = mysqli_query($con,$sql6);
            while(list($ngsecond)=mysqli_fetch_row($rs6))
            {
                $ngs_sum+=(float)$ngsecond;
            }
            $tmp=number_format(($ngs_sum*100/(int)$usetime),2);
        ...
        while(list($uu)=mysqli_fetch_row($rs)){
            ...
            $data1 = array();
            $idx++;
            while(list($starttime,$usetime,$ngtimes)=mysqli_fetch_row($rs4))
            {
                $label[$i] = $i+1;
                $tmp=0;
                if($ngtimes!=0){
                    $ngs_sum=0;
                    $sql7 = "select ngsecond from slogtb where user = '".$uu."' and starttime = '".$starttime."'";
                    $rs7 = mysqli_query($con,$sql7);
                    while(list($ngsecond)=mysqli_fetch_row($rs7))
                    {
                        $ngs_sum+=(float)$ngsecond;
                    }
                    $tmp=number_format(($ngs_sum*100/(int)$usetime),2);
